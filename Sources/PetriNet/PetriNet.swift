...
where PlaceType: Place, TransitionType: Transition
{
    ...
    public typealias ArcLabel = PlaceType.Content
    ...
    public struct ArcDescription {
        ...
        /// The arc's label.
        fileprivate let label: PlaceType.Content
        ...
        public static func pre(
            ...
            labeled label: PlaceType.Content = PlaceType.Content.successor(of: .zero)
        ) -> ArcDescription {
        ...
        public func getTrans() -> TransitionType{
            return transition
        }
    ...
    public struct PartialArcDescription {
        ...
        fileprivate init(place: PlaceType, label: PlaceType.Content, isPre: Bool) {
            self.place = place
            self.label = label
            self.isPre = isPre
        }
    ...
    ///
    /// - Returns: The marking that results from the firing of the given transition if it is
    ///   fireable, or `nil` otherwise.
...
public protocol Place: CaseIterable, Hashable, Comparable {
    
    associatedtype Content: AdditiveArithmetic & PeanoRepresentable & Hashable & Comparable
